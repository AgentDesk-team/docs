...
You can create work from chat or from the board.

### From chat

Ask an agent or assistant to create the task:

```text
Create a task for this request and keep it in inbox until I approve it.
```

This is useful when the request still needs shaping.

### From the board

Use the task creation flow when you already know the goal, output, and acceptance criteria.

After creating the task, open Task Detail and check that the task has:

| Field | What to check |
| --- | --- |
| Title | Short and action-oriented. |
| Summary | Explains the outcome, not only activity. |
| Status | Usually starts in **Inbox** unless you already approved execution. |
