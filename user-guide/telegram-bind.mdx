...
## If Telegram is connected but replies are not flowing yet

Check these in order:

1. Make sure you messaged the bot from the Telegram user that is allowed for the workspace.
2. Reopen the Telegram dialog and confirm the bot identity is still correct.
3. Send a fresh message to the bot after binding instead of relying only on token validation.
4. If AgentDesk shows setup still pending, finish the remaining activation step and retry.
5. If the Dashboard or workspace runtime is still serving old state, reload the page or restart the workspace gateway, then test again.
